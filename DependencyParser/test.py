...
if __name__ == '__main__':
    ...
    #get swedish training data
    swedishdata = dataset.get_swedish_train_corpus().parsed_sents()
    random.seed(1234)
    swedishsubdata = random.sample(swedishdata, 222)
    
    #get english training and testing data
    #training is done on sentences 200-400 in training file
    #testing is done on sentences 400-600
    englishdata = dataset.get_english_train_corpus().parsed_sents()
    englishsubdata = []
    englishtestdata = []
    for i in range (200, 400):
        englishsubdata.append(englishdata[i])
        englishtestdata.append(englishdata[i+400])
    ...
    try:
        ...
        tp.train(englishsubdata)
        tp.save('english.model')
        
         
        parsed = tp.parse(englishtestdata) 
        ev = DependencyEvaluator(englishtestdata, parsed)
